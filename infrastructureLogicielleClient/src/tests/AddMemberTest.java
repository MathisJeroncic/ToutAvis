...
public class AddMemberTest {
	 ...
	 * BadEntry exception and does not change content of the <i>ISocialNetwork</i>.
	 * If OK, the method just returns 0. If not OK, displays an error message and
	 * returns 1.
	 * 
	 ...
	 * @param testId       - the test ID that will prefix any error message
	 *                     displayed by this method
	 * @param errorMessage - the error message that will be displayed if no
	 *                     exception is thrown when adding this member
	 * @return 0 if the test is OK, 1 if not
	 */
	private static int addMemberBadEntryTest(ISocialNetwork sn, String login, String pwd, String profile, String testId,
			String errorMessage) {
	 ...
	 * <i>ISocialNetwork</i>. If OK, the method just returns 0. If not OK, displays
	 ...
	 * @return 0 if the test is OK, 1 if not
	 */
	private static int addMemberAlreadyExistsTest(ISocialNetwork sn, String login, String pwd, String profile,
		...
		try {
			...
			if (sn.nbMembers() != nbMembers) {
				System.out.println(
						"Err " + testId + " : MemberAlreadyExists was thrown, but the number of members was changed"); // Display
																														// a
																														// specific
																														// error
																														// message
	...
	/**
	 * Check that this new member (login, pwd, profile) can be (and <i>is</i>) added
	 * to the <i>ISocialNetwork</i>.</br>
	 * If OK, the method just returns 0 : the new member was added.</br>
	 * If not OK, an error message is displayed and 1 is returned ; the new member
	 ...
	 * @return 0 if the test is OK, 1 if not
	 */
	private static int addMemberOKTest(ISocialNetwork sn, String login, String pwd, String profile, String testId) {
		int nbMembers = sn.nbMembers(); // Number of members when starting to
										// process this method
		try {
			sn.addMember(login, pwd, profile); // Try to add this member
			...
			if (sn.nbMembers() != nbMembers + 1) { // But the number of members
													...
													// accordingly
				System.out
						.println("Err " + testId + " : the number of members (" + nbMembers + ") was not incremented"); // Error
																														// message
																														// displayed
	...
	public static TestReport test() {
		...
		nbTests++;
		nbErrors += addMemberBadEntryTest(sn, null, "qsdfgh", "", "1.1", "addMember() doesn't reject null logins");
		nbTests++;
		...
		nbTests++;
		nbErrors += addMemberBadEntryTest(sn, "B", "   qwd ", "", "1.4",
		...
		nbTests++;
		nbErrors += addMemberAlreadyExistsTest(sn, new String("Paul"), "abcdefghij", "", "2.2",
				"The login of the first member was accepted as login for a new member");
		nbTests++;
		nbErrors += addMemberAlreadyExistsTest(sn, new String("Alice"), "abcdefghij", "", "2.3",
				"The login of the last member was accepted as login for a new member");
		nbTests++;
		nbErrors += addMemberAlreadyExistsTest(sn, new String("anToine"), "abcdefghij", "", "2.4",
				"An already registered login, but with different case, was accepted as login for a new member");
		nbTests++;
		nbErrors += addMemberAlreadyExistsTest(sn, new String(" Antoine "), "abcdefghij", "", "2.5",
				"An already registered login, surrounded by leading/trailing blanks, was accepted as login for a new member");
		nbTests++;
		nbErrors += addMemberAlreadyExistsTest(sn, "An" + "toi" + "ne", "abcdefghij", "", "2.6",
