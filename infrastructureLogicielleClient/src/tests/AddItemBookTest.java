...
public class AddItemBookTest {	
	 ...
	 */
	private static int addItemBookBadEntryTest(ISocialNetwork sn, String login, String pwd, String title, String kind,
	 ...
	 * @return 1 if the test fails, 0 otherwise.
	 */
	private static int addItemBookAlreadyExistsTest(ISocialNetwork sn, String login, String pwd, String title,
	 ...
	 * @return 1 if the test fails, 0 otherwise.
	 */
	private static int addItemBookOKTest(ISocialNetwork sn, String login, String pwd, String title, String kind,
			String author, int nbPages, String testId) {
		int nbBooks = sn.nbBooks();
		try {
			sn.addItemBook(login, pwd, title, kind, author, nbPages);
			if (sn.nbBooks() != nbBooks + 1) {
				System.out.println("Err " + testId + " : the number of members (" + nbBooks + ") was not incremented");
				return 1;
			} else
				return 0;
		} catch (Exception e) {
	 ...
	 * @return 1 if the test fails, 0 otherwise.
	 */
	private static int addItemBookNotMemberTest(ISocialNetwork sn, String login, String pwd, String title, String kind,
			String author, int nbPages, String testId, String errorMessage) {
		int nbBooks = sn.nbBooks();
		try {
			sn.addItemBook(login, pwd, title, kind, author, nbPages);
			...
			if (sn.nbBooks() != nbBooks) {
				System.out.println(
						"Err " + testId + " : NotMemberException was thrown, but the number of Book was changed");
				return 1;
			} else
				return 0;
		} catch (Exception e) {
	...
	public static TestReport test() throws BadEntryException, MemberAlreadyExistsException {
		...
		nbTests++;
		nbErrors += addItemBookOKTest(sn, "Paul", "paul", "One piece", "Manga", "ODA", 50, "2.1a");
		nbTests++;
		nbErrors += addItemBookOKTest(sn, "Antoine", "antoine", "code en java pour les nulls", "Tuto", "pour les nulls",
				1150, "2.1b");
		nbTests++;
		nbErrors += addItemBookOKTest(sn, "Janne", "uoiu", "reussir l'imt en 2 pages", "tuto", "Fip", 2, "2.1c");
		...
		nbErrors += addItemBookAlreadyExistsTest(sn, "Paul", "paul", new String("One piece"), "Manga",
				new String("ODA"), 50, "2.2",
				"The title and the author of the first book was accepted as title for a new book");
		nbTests++;
		nbErrors += addItemBookAlreadyExistsTest(sn, "Janne", "uoiu", new String("reussir l'imt en 2 pages"), "tuto",
				new String("Fip"), 2, "2.3",
				"The title and the author of the last book was accepted as title for a new book");
		nbTests++;
		nbErrors += addItemBookAlreadyExistsTest(sn, "Antoine", "antoine", new String("Code En jaVa pour lEs nulls"),
				"Tuto", new String("pour lEs nullS"), 1150, "2.4",
				"An already registered book, but with different case, was accepted as title and author for a new book");
		nbTests++;
		nbErrors += addItemBookAlreadyExistsTest(sn, "Antoine", "antoine", new String("Code En jaVa pour lEs nulls "),
				"Tuto", new String(" pour lEs nullS"), 1150, "2.5",
				"An already registered login, surrounded by leading/trailing blanks, was accepted as login for a new member");
		nbTests++;
		nbErrors += addItemBookAlreadyExistsTest(sn, "Antoine", "antoine", "code" + " en java" + " pour les nulls",
				"Tuto", "pour" + " les nullS", 1150, "2.6",
		...
		nbErrors += addItemBookNotMemberTest(sn, "Victor", "STETER", "Les miserable", "Roman", "Victor hugo", 1630,
				"3.1", "The book has been added, but the member does not exist");
		nbTests++;

		nbErrors += addItemBookNotMemberTest(sn, "Paul", "STETER", "Les miserable", "Roman", "Victor hugo", 1630, "3.2",
